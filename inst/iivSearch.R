...
testfun <- function(x, fix, focei) {
  ...
  list(
    rank = which("etaTcl+etaTv+etaTcl~etaTv" == passed$search[order(passed$BIC)]),
...
ggplot(data.frame(rank = as.factor(unlist(lapply(res_fixed, \(x) x[["rank"]]))))) +
  ...
  labs(y = "Frequency", title = "FOCE+I Linearization", x = "True BSV Rank") +
  theme_classic()


ggplot(data.frame(rank = as.factor(unlist(lapply(res_fixed_foce, \(x) x[["rank"]]))))) +
  geom_bar(aes(x = rank, y = (..count..) / sum(..count..))) +
  labs(y = "Frequency", title = "FOCE Linearization", x = "True BSV Rank") +
  theme_classic()
...
fit.prop <- one.cmpt.adderr |> model(cp ~ prop(prop.sd)) |> nlmixr(nlmixr2data::theo_sd, est = "focei")

fit.combined2 <- one.cmpt.adderr |>
  model(cp ~ add(add.sd) + prop(prop.sd) + combined2()) |>
  nlmixr(nlmixr2data::theo_sd, est = "focei")

fit.combined1 <- one.cmpt.adderr |>

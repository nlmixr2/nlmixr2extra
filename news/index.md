...
- `theoFitOde` is regenerated by
  [`devtools::document()`](https://devtools.r-lib.org/reference/document.html)
  again, and the stored fit no longer needs `qs2` to read. The
  `@eval .buildModel()` hook that rebuilt it was removed by accident in
  Jan 2023, so the shipped fit still had `origData`/`parHistData`
  serialized with `qs2` – a format written only by rxode2 versions where

...
one.compartment <- function() {
  ini({
    tka <- log(1.6) ; label("log Ka")
    tcl <- log(2.7) ; label("Log Cl")
    tv <- log(32) ; label("Log V")
    eta.ka ~ 0.6
    eta.cl ~ 0.3
    eta.v ~ 0.1
    add.sd <- 0.7
...
fit <- nlmixr2(one.compartment, data = nlmixr2data::theo_sd,  est = "saem", control = saemControl(print=0))

# In a real example, nboot would likely be 500 or more.
fitBoot <- suppressMessages(bootstrapFit(fit, nboot = 5))
fitBoot
```

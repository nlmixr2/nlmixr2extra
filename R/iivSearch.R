...
iivSearch.nlmixr2Linearize <- function(fit, sortBy = "BIC", mceta = 5, ...) {
  ...
  if (!hasAnyEta(fit)) {
    stop("Use `addEtas=TRUE` while linearizing this model")
  }
  # get eta names
  ...
  res <- lapply(cli::cli_progress_along(iivSpace, "IIV Search"), function(x) {
    ...
    unfixedIni$upper[!is.na(unfixedIni$ntheta) & unfixedIni$fix == FALSE] <- unfixedIni$est[
      !is.na(unfixedIni$ntheta) & unfixedIni$fix == FALSE
    ] *
      2 # set upper to residual to 2x
...
addAllEtas <- function(ui, fix = FALSE) {
  ...
  if (!is.null(ui$noMuEtas)) {
    stop("Some Etas are not mu referenced")
  }
  if (!inherits(ui, "rxUi")) {
    stop("`ui` must be a model or fit")
  }

  ...
  for (f in seq_len(nfold)) {
    ...
    if (dim(testData)[1] == 0) {
      cli::cli_alert_danger(
        "the test dataset is empty. probably not enough individuals to do desired n-fold cross-validation"
      )
      stop("aborting...please re-run by reducing nfold number for the cross-validation", call. = FALSE)
    }

    cli::cli_alert_success(
      "Training and Testing data sets successfully created for cross-validation for fold number {f}"
    )
    ...
    if (optcrit == "objf") {
      if (!is.numeric(fitTest$objDf$OBJF)) {
        ...
        cli::cli_alert_info(
          "try computing 'fit$objDf$OBJF' in console to compute and store the corresponding OBJF value"
        )
      ...
      if (!is.numeric(fitTest$objDf$`Log-likelihood`)) {
        cli::cli_alert_danger("the 'fit' object needs to have an objective functions value associated with it")
        cli::cli_alert_info(
          "try computing 'fit$objDf$$`Log-likelihood`' in console to compute and store the corresponding OBJF value"
        )
        stop("aborting...objf value not associated with the current 'fit' object", call. = FALSE)
      }
...
.optimalTvaluelasso <- function(
  ...
  pofvList <- data.frame()
  for (t in tvalues) {
    ofValue <- suppressWarnings(.crossvalidationLasso(
      data,
      ui,
      varsVec,
      covarsVec,
...
regularmodel <- function(
  ...
  if (lassotype == "regular") {
    ...
    .coefValues <- adaptivelassoCoefficients(
      fit,
      varsVec,
      covarsVec,
      catvarsVec,
    ...
    .coefValues <- adjustedlassoCoefficients(
      fit,
      varsVec,
      covarsVec,
      catvarsVec,
...
adjustedlassoCoefficients <- function(fit, varsVec, covarsVec, catvarsVec, constraint = 1e-08, stratVar = NULL, ...) {
  ...
  .adapcoefs <- data.frame(as.list(
    fit1$parFixedDf[row.names(fit1$parFixedDf) %in% covNames, "Estimate"] /
      fit1$parFixedDf[row.names(fit1$parFixedDf) %in% covNames, "SE"]
  ))

...
.preCondModExtra <- function(pre, d0) {
  ...
  .lines <- vapply(
    ...
    function(.i) {
      ...
      .terms <- paste0("(", vapply(.coef[.keep], .preCondNum, character(1)), ")*", .d[.keep])
...
preconditionFit <- function(fit, estType = c("full", "posthoc", "none"), ntry = 10L) {
  nlmixrWithTiming(
    ...
    {
      if (!exists("R", fit$env)) {
        stop("this assumes a covariance matrix with a R matrix", call. = FALSE)
      }
      .R <- fit$R
      .covMethod <- ""
      .i <- 1
      ...
      if (!.preCondIsRS(.covMethod)) {
        stop("preconditioning failed after ", ntry, " tries", call. = FALSE)
      }

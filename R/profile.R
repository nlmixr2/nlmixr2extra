...
profileFixedSingle <- function(fitted, which) {
  ...
  cli::cli_inform("Profiling {paste(names(which), '=', unlist(which), collapse = ', ')}")
...
profileLlp <- function(fitted, which, control) {
  ...
  results <- lapply(which, function(w) llpRunOneParameter(fitted = fitted, which = w, control = control))
...
llpNextParam <- function(paramVals, ofvVals, targetOfv, direction, mle,
                         searchLower, searchUpper, hardLower, hardUpper) {
  # Remove NAs and restrict to active half-space
  keep <- !is.na(ofvVals) & (if (direction < 0) paramVals <= mle else paramVals >= mle)
  x <- paramVals[keep]
  y <- ofvVals[keep]
  ...
  if (n >= 3L) {
    ...
    if (!is.null(fit)) {
      ...
      if (!is.finite(c2) || abs(c2) < 1e-10) {
        cli::cli_warn("Flat quadratic profile; falling back to linear interpolation/extrapolation.")
      } else {
        ...
        if (is.finite(disc) && disc >= 0) {
          ...
          valid_r <- roots[
            is.finite(roots) & roots >= searchLower & roots <= searchUpper & hs_ok
          ]
  ...
  if (is.na(result)) {
    if (targetOfv >= min(y) && targetOfv <= max(y)) {
      ...
      if (direction < 0) {
        xo <- x[1L]; yo <- y[1L]; xi <- x[2L]; yi <- y[2L]
      } else {
        xo <- x[n]; yo <- y[n]; xi <- x[n - 1L]; yi <- y[n - 1L]
      }
      dxdy <- (xo - xi) / (yo - yi)
      if (!is.finite(dxdy) || dxdy == 0) return(NA_real_)
...
optimProfile <- function(par, fitted, optimDf, which, ofvIncrease, direction,
                         searchLower, searchUpper, hardLower, hardUpper, mle,
                         fixFn = NULL,
  ...
  while (itermax > currentIter) {
    ...
    if (isTRUE(any(currentMinOFV < origMinOFV))) {
      cli::cli_warn("OFV decreased while profiling {which}; profile may be inaccurate")
      origMinOFV <- NA
    }
    ...
    signifSame      <- signif(ret[[which]], digits = paramDigits) == signif(nextPar, digits = paramDigits)
    convergedSignif <- any(signifSame & ret[[which]] > nextPar) && any(signifSame & ret[[which]] < nextPar)
    ...
    if (converged) {
      ...
      storeBound     <- data.frame(Parameter = which, X = nextPar, profileBound = direction * ofvIncrease)
      names(storeBound)[2L] <- which
      ret <- dplyr::bind_rows(storeBound, ret)
      break
    }

    cli::cli_inform("Profiling {which} best distance to target OFV: {round(currentOFVDiff, 4)}")
    currentPar <- nextPar
  }
...
llpRunOneParameter <- function(fitted, which, control) {
  ...
  effectRange <- fitted$iniDf[fitted$iniDf$name == which, c("lower", "upper"), drop = FALSE]
  ...
  if (length(lBound) == 1L && length(uBound) == 1L && !is.na(lBound) && !is.na(uBound)) {
...
llpOmegaNames <- function(fit) {
  idf <- fit$iniDf
  omega_rows <- idf[is.na(idf$ntheta) & !is.na(idf$neta1) & idf$neta1 == idf$neta2, ]
...
buildFixedOmegaModel <- function(fit, omegaName, fixOmegaVal) {
  ...
  omegaRows  <- idf[is.na(idf$ntheta) & !is.na(idf$neta1) & idf$neta1 == idf$neta2, ]
  ...
  modelLines <- vapply(ui$lstExpr, function(e) paste0("    ", deparse(e)), character(1L))
  fnText <- paste0(
    "function() {\n  ini({\n",
...
profileFixedOmegaSingle <- function(fitted, omegaName, fixOmegaVal, fitData) {
  ...
  newFit  <- try(suppressMessages(
    nlmixr2est::nlmixr2(modelFn, data = fitData, est = fitted$est,
                        control = setQuietFastControl(fitted$control))
  ))
  ret <- profileNlmixr2FitCoreRet(fitted = newFit, which = omegaName,
...
llpRunOneOmega <- function(fitted, which, control) {
  ...
  if (!is.null(names(rse))) rse <- rse[[which]]
  if (is.na(se)) {
  ...
  if (lowerGuess <= hardLower) lowerGuess <- (hardLower - mle) * 0.9 + mle
  if (upperGuess >= hardUpper) upperGuess <- (hardUpper - mle) * 0.9 + mle
  ...
  fixFn      <- function(fixOmegaVal) {
    profileFixedOmegaSingle(fitted    = fitted,
                            omegaName = which,
...
llpControl <- function(ofvIncrease       = qchisq(0.95, df = 1),
                       ...
                       workers           = NULL) {
  checkmate::assert_numeric(rseTheta, lower = 0, finite = TRUE, any.missing = FALSE, min.len = 1)
  ...
  if (!isUnnamedScalar && !isNamedVector) {
    cli::cli_abort("{.arg rseTheta} must be a single unnamed number or a named numeric vector")
  }

  if (!is.null(workers) && !identical(workers, "auto")) {
    checkmate::assert_integerish(workers, lower = 1, any.missing = FALSE, len = 1, null.ok = FALSE, coerce = TRUE)
  ...
  ret <-
    list(
      ...
      normq             = checkmate::assert_number(normq, lower = 0, finite = TRUE, null.ok = FALSE, na.ok = FALSE),
      rseTheta          = rseTheta,
...
profileNlmixr2FitDataEstInitial <- function(estimates, which, normq, rseTheta, seTheta, hardLower, hardUpper) {
...
.llpCheckParallelDeps <- function() {
  ...
  if (!requireNamespace("future",       quietly = TRUE)) missing_pkgs <- c(missing_pkgs, "future")
  if (!requireNamespace("future.apply", quietly = TRUE)) missing_pkgs <- c(missing_pkgs, "future.apply")
...
messageProfileComplete <- function(which, direction, msg) {
  dirStr <- if (direction > 0L) "above the point estimate" else "below the point estimate"
...
.nlmixr2ProfileNew <- function(raw, status, fit, which, control, fitName = NA_character_) {
  fixefNames <- names(nlmixr2est::fixef(fit))
  omegaNames <- llpOmegaNames(fit)
  mle <- setNames(vapply(which, function(w) {
    if (w %in% fixefNames) {
...
runLLP <- function(fit, which = NULL, control = llpControl(), ...) {
  fitName <- tryCatch(deparse(substitute(fit)), error = function(e) NA_character_)
  ...
  if (!is.null(workers) && !identical(workers, 1L)) {
    ...
    cli::cli_inform("Profiling {length(which)} parameter{?s} in parallel (workers = {workers})")
    results <- .withWorkerPlan(workers, {
      future.apply::future_lapply(which, function(w) {
        llpWithIsolatedFitDir(w, function() { runOne(w) })
      }, future.seed = TRUE, future.packages = "nlmixr2extra")
    })
  } else {
    results <- lapply(seq_along(which), function(i) {
      cli::cli_inform("Profiling parameter {i}/{length(which)}: {.field {which[i]}}")
      runOne(which[i])
    })
  }
  ...
  keepCols <- c("Parameter", "OFV",
                intersect(c(fixefNames, whichOmega), names(raw)),
                if ("profileBound" %in% names(raw)) "profileBound")
  raw <- raw[, keepCols, drop = FALSE]
  if (!"profileBound" %in% names(raw)) raw$profileBound <- NA_real_

  .nlmixr2ProfileNew(raw = raw, status = status, fit = fit, which = which,
                     control = control, fitName = fitName)
}
...
print.nlmixr2Profile <- function(x, ...) {
  ...
  cli::cli_text("{.field ΔOFV} target: {round(ofvIncrease, 4)}")
  cli::cli_text("")
  ...
  for (p in params) {
    lo    <- if (!is.na(ci[p, "lower"])) formatC(ci[p, "lower"], digits = 4L, format = "f") else "      NA"
    hi    <- if (!is.na(ci[p, "upper"])) formatC(ci[p, "upper"], digits = 4L, format = "f") else "      NA"
    rat   <- if (!is.na(intervalRatio[p])) formatC(intervalRatio[p], digits = 2L, format = "f") else "    NA"
    flags <- character(0L)
    if (isTRUE(nearBound[p]))  flags <- c(flags, "near bound")
    if (isTRUE(hitMaxIter[p])) flags <- c(flags, "max iter")
    cli::cli_text(sprintf(fmt, p, lo, formatC(mle[[p]], digits = 4L, format = "f"), hi, rat,
...
confint.nlmixr2Profile <- function(object, parm = NULL, level = 0.95, ...) {
  ...
  mat <- matrix(NA_real_, nrow = length(params), ncol = 2L,
  ...
  for (p in params) {
    ...
    if (length(lo) > 0L && !is.na(lo[[1L]])) mat[p, "lower"] <- lo[[1L]]
...
plot.nlmixr2Profile <- function(x, parm = NULL, ...) {
  ...
  plotList <- lapply(params, function(p) {
    ...
    data.frame(param = p, paramVal = rows[[p]], dOFV = rows$OFV - minOFV,
               stringsAsFactors = FALSE)
  })
  plotData <- dplyr::bind_rows(plotList)

  vlineList <- lapply(params, function(p) {
    xvals <- c(MLE = mle[[p]], `CI lower` = ci[p, "lower"], `CI upper` = ci[p, "upper"])
    xvals <- xvals[!is.na(xvals)]
    data.frame(param = p, xval = unname(xvals), linetype = names(xvals),
               stringsAsFactors = FALSE)
  })
  vlineData <- dplyr::bind_rows(vlineList)

  ggplot2::ggplot(plotData, ggplot2::aes(x = .data[["paramVal"]], y = .data[["dOFV"]])) +
    ggplot2::geom_line() +
    ggplot2::geom_point(size = 1.5) +
    ggplot2::geom_hline(yintercept = ofvIncrease, linetype = "dashed", colour = "grey50") +
    ggplot2::geom_vline(
      data = vlineData,
      ggplot2::aes(xintercept = .data[["xval"]], linetype = .data[["linetype"]]),
      colour = "grey30"
    ) +
    ...
    ggplot2::labs(x = "Parameter value", y = expression(Delta * "OFV"), linetype = NULL) +
    ggplot2::theme_bw()
}
